...
content_markdown: |-
    **请求参数**\:
    ...
    | direction | 滚动分页（NEXT,PREV） | query | false | string | &nbsp; |
    | endTime | 结束时间 | query | false | integer(int64) | &nbsp; |
    | id | orderid | query | false | integer(int64) | &nbsp; |
    | limit | 条数 | query | false | integer(int32) | &nbsp; |
    | startTime | 起始时间 | query | false | integer(int64) | &nbsp; |
    | symbol | 交易对 | query | false | string | &nbsp; |
    | type | EXCHANGE:划转;CLOSE_POSITION:平仓盈亏;TAKE_OVER:仓位接管;QIANG_PING_MANAGER:强平清算费（手续费）;FUN:资金费用;FEE:手续费 (开仓、平仓、强平);ADL:自动减仓MERGE:仓位合并 | query | false | string | &nbsp; |
